...
4. **THE SACRED PATTERNS** (from Phase 2's + Batch 3's security review — copy wherever they apply):
   ...
   - **FK actor guard**: every write binding a ctx-supplied userId to a UUID column must wrap it in `uuidOrNull()` (synthetic `api-key:<name>` / `system` actors become NULL). Applies to actor columns (created_by/updated_by/connected_by/replied_by/banned_by/author_id) AND subject columns on `optional`/`public`-tier writes where an API key can authenticate (e.g. contact_messages.user_id, form_submissions.user_id — also any `WHERE uuid_col = $userId` comparison). Import `uuidOrNull` from `../utils/uuid`.
...
- [ ] Write `docs/client-sdk-plan.md` — NOT an implementation, a charter for the next project: goals (typed TS client mirroring cms.* shape over HTTP), inputs (docs/api-manifest.json + @rw/shared types + ErrorCode/AuthTier), proposed package layout (@sitesurge/client or @sitesurge/sdk; fetch-based, zero deps), auth handling (Bearer JWT w/ refresh + API-key mode), generation strategy options (hand-rolled thin client vs manifest-driven codegen — manifest now carries enough for codegen of method/path/auth; input/output TYPES still need per-module DTO exports in shared/src/api/routes/<module>.ts, which only posts has → list this as the main prerequisite gap), error mapping (ApiError → typed exceptions switchable on ErrorCode), pagination conventions, the CONTENT_LOCKED details contract, browser+node compatibility notes, versioning policy suggestion, and any notes accumulated during the sweep (each batch implementer reports SDK-relevant observations; collect them here).
- [ ] Commit.
