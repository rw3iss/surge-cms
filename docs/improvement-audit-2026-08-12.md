...
- **Phase C — APPLIED (autonomous multi-agent waves; full build ✓ + 170 API tests ✓):**
  - **A7 registries (agent):** `services/search.ts` rewritten to a `SEARCHABLE_ENTITIES`/`ADMIN_SEARCHABLE` registry + one generic `searchEntity` (+2 tests); `social.ts` Twitter special-casing folded into its `PROVIDERS.twitter` entry via `readCursor`/`writeCursor`/`isSyncEnabled` (`syncSocialPosts` now provider-agnostic); `oauth/index.ts` switch → `OAUTH_PROVIDERS` map. Behavior identical.
  - **UI kit (agent):** `.ui-button` is now the canonical admin button — standalone class (works on `<button>` + `<a>`), sizes `--sm/--md/--lg`, colors `--primary/--secondary/--ghost/--danger/--success/--warning`, styled to match the old `.btn`. Added a `Badge` component (`.ui-badge`) + a controlled `Tabs` API.
  - **`<DataTable>` (me + agent):** new `components/admin/common/DataTable.tsx` (columns config + bulk bar + sort + pagination + loading/empty + optional `onRowClick`/`rowClass`), driven by the existing hooks. **Migrated all 6 core list pages** (Posts, Pages, Campaigns, Forms, Messages, Users) onto it — each page's table scaffold collapsed to a column config.
  - **Button adoption (me):** token-exact sweep migrated **274 admin `.btn` usages → `.ui-button`** (30 non-standard `btn--icon/outline/tiny/xs/link/danger-*` left as `.btn`; template-literal/dynamic classes untouched); `ConfirmModal` now uses the `<Button>` component.
  - **Deferred (documented):** the 30 non-standard `.btn` variants (need `.ui-button` equivalents), template-literal button classes, `<Badge>` adoption, and the `ui/FormField` ↔ `admin/forms/FormField` kit merge + 8 `pages/setup/*` migration (skipped to avoid risking the first-run wizard).

- **(superseded) originally planned:**
