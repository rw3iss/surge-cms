...
- **Content-block preview containment** — `.content-block__preview-body` is `position: relative` (the positioning context for a block's inner content) so a block whose markup uses `position:absolute; inset:0` (e.g. a full-bleed custom-HTML block) resolves against the body — which sits BELOW the header bar — instead of the whole `.content-block`, where it would paint up over the `.content-block__hover-bar` header and steal its clicks. Non-selected custom-HTML previews (`.content-block--html:not(.content-block--selected) > .content-block__preview-body`) also `overflow: hidden` to clip arbitrary markup to the block; group/slot bodies stay `overflow: visible` so their hover menus aren't clipped.
