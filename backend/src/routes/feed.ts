 ...
 *
 * Mounted at `/feed.xml` and `/api/v1/feed.xml`. Cached in Redis for
 * 30 minutes — long enough that a heavy aggregator polling every 5
 * minutes is mostly hitting cache, short enough that newly-published
 * posts appear in feeds within half an hour without an explicit
 * invalidation hook.
 *
 ...
 * content). Content is sanitized + truncated for safety since some
 * readers render it inline.
 */
import { Request, Response, Router, } from 'express';
import { config, } from '../config';
import { query, } from '../db';
import { cache, } from '../services/cache';
import { stripHtml, truncateText, } from '../services/ssr/schema';
...
router.get('/feed.xml', async (_req: Request, res: Response,) => {
    try {
        ...
        // Empty-but-valid feed on error so aggregators don't blacklist
        // the URL during a transient DB outage.
        res.status(500,).set('Content-Type', 'application/rss+xml; charset=utf-8',).send(
