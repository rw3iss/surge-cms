import { z, } from 'zod';
import * as providerWebhooks from '../services/shop/providers/webhooks';
import type {
    AssertCompatible,
...
const checkoutPreviewSchema = z.object({
    ...
    shippingMethod: z.union([z.string(), z.record(z.string(), z.string(),),],).optional(),
...
export const shopRoutes = [
    ...
    // ─── Provider webhooks (public, token in the path) ────────────
    //
    // Declared FIRST so a `/:slug`-shaped route later in this module cannot
    // shadow them. `raw: true` because a signed payload has to be verified
    // against the exact bytes received, not a re-serialised object.
    //
    // The URLs are generated by us and pasted into the provider's dashboard —
    // see services/shop/providers/settings.ts#webhookUrl.
    defineRoute({
        method: 'post', path: '/webhooks/:provider/:path/:token', auth: 'public', raw: true,
        summary: 'Inbound provider webhook (product add/update, fulfilment, …).',
        input: { params: webhookParams, },
        handler: async ({ params, req, res, },) => {
            const result = await providerWebhooks.handleWebhook({
    ...
    defineRoute({
        method: 'get', path: '/webhooks/:provider/:path/:token', auth: 'public', raw: true,
        summary: 'Inbound provider webhook, GET form (Apliiq product search uses ?search=).',
        input: { params: webhookParams, },
        handler: async ({ params, req, res, },) => {
            const result = await providerWebhooks.handleWebhook({
                provider: params.provider,
