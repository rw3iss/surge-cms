 ...
 * charge £0 is pure friction. PAID tickets are priced here from the database
 * and handed back for the Stripe step; tickets are only issued once payment has
 * succeeded, so an abandoned checkout never holds inventory.
 */
export async function purchaseTickets(input: {
    lines: import('@sitesurge/types').EventTicketCartLine[];
    email: string;
    name?: string;
    phone?: string;
    userId?: string;
    ...
    const { resolveTicketLines, issueTickets, } = await import('./events/tickets.js');
    ...
    const eventIds = [...new Set(lines.map((l: { eventId: string; },) => l.eventId),),];
    ...
    return {
        ...
        tickets: tickets.map((t: { code: string; tierName: string; },) => ({ code: t.code, tierName: t.tierName, }),),
    };
}
